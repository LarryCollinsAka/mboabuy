...
class InstallController extends Controller
{
    ...
    {
        // 1. Lift the memory limit and execution time for this heavy process
        ini_set('memory_limit', '1024M');
        set_time_limit(300); // Give it 5 minutes to run
        try {
            // Import Base SQL
            $sql_path = base_path('shop.sql');
            ...
            if (file_exists($demo_sql_path)) {
                // Note: If this file is massive, this line is what triggers the 500 error
                DB::unprepared(file_get_contents($demo_sql_path));
            }

            // 2. Extract images (This can also be memory intensive)
            $zip_path = base_path('public/uploads.zip');
            if (file_exists($zip_path)) {
                $zip = new ZipArchive;
                if ($zip->open($zip_path) === TRUE) {
                    $zip->extractTo(base_path('public/uploads/all/'));
                    $zip->close();
                }
            }
            ...
            // If it fails, we want to see the actual error message
            return $e->getMessage();
        }
    }
